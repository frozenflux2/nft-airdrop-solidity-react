...
async function updateAbi() {
    const nftAirdrop = await ethers.getContract("NFTAirdrop");

    fs.writeFileSync(
        FRONTEND_ABI_FILE_PATH,
        JSON.parse(
            JSON.stringify(
                nftAirdrop.interface.format(ethers.utils.FormatTypes.json)
            )
        )
    );
}
...
async function updateContractAddresses() {
    // * get the contract.
    const nftAirdrop: NFTAirdrop = await ethers.getContract("NFTAirdrop");
    ...
    if (chainId != undefined) {
        ...
        if (chainId in contractAddresses) {
            ...
            contractAddresses[network.config.chainId!] = [nftAirdrop.address];
        }
        fs.writeFileSync(
